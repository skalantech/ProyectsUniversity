...
void ListeDeCases::ajouterCase(const Case& uneCase, int position) {
    ...
    auto nouveau = std::make_shared<Noeud>(uneCase); // Noeud Crée
 ...
 */
std::shared_ptr<ListeDeCases::Noeud> ListeDeCases::noeudAt(int position) const {
    // Corrigé
    if (position < 1 || position > taille()) {
 ...
 */
std::shared_ptr<ListeDeCases::Noeud> ListeDeCases::Iterateur::noeudSuivant(Case::Couleur couleur, int& distance) const {
    distance = 0;
    if (m_courant == 0) {
        return 0;
    }

    std::shared_ptr<ListeDeCases::Noeud> suivant = m_courant->m_suivant;
 ...
 */
bool ListeDeCases::Iterateur::noeudMauvaiseCouleur(std::shared_ptr<ListeDeCases::Noeud> noeud, Case::Couleur couleur) const {
 ...
 */
std::shared_ptr<ListeDeCases::Noeud>  ListeDeCases::Iterateur::noeudPrecedent(Case::Couleur couleur, int& distance) const {
    distance = 0;
    if (m_courant == 0) {
        return 0;
    }

    std::shared_ptr<ListeDeCases::Noeud> precedent = m_courant->m_precedent;
...
int ListeDeCases::Iterateur::positionCourante() const {
    std::shared_ptr<Noeud> courant = m_liste.m_debut;
    if (courant == 0)
