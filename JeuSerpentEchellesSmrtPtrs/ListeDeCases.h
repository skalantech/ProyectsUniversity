...
private:
    ...
    struct Noeud {
        //Constructeur
        Noeud(const Case &uneCase, std::shared_ptr<Noeud> suivant = nullptr, std::shared_ptr<Noeud> precedent = nullptr) :
        ...
        std::shared_ptr<Noeud> m_suivant;

        //Un pointeur vers le noeud précédent de la liste.
        std::shared_ptr<Noeud>  m_precedent;
    };
...
public:
    ...
    private: //Implantation de l'itérateur.
        ...
        std::shared_ptr<Noeud> m_courant;

        std::shared_ptr<Noeud> noeudSuivant(Case::Couleur couleur, int &distance) const;

        bool noeudMauvaiseCouleur(std::shared_ptr<Noeud> noeud, Case::Couleur couleur) const;

        std::shared_ptr<Noeud> noeudPrecedent(Case::Couleur couleur, int &distance) const;
...
private://Implantation de la liste de cases par une structure doublement chaînée.
    ...
    std::shared_ptr<Noeud> m_debut;

    //Pointeur sur le dernier noeud de la liste.
    std::shared_ptr<Noeud> m_fin;
    ...
    //Lance std::range_error si la position est hors borne.
    std::shared_ptr<Noeud> noeudAt(int position) const;
};
